...
// Mock analysis function (used for fallback or testing)
async function analyzeWithAI(content: string): Promise<AnalysisResult> {
...
export async function callGeminiAPI(content: string): Promise<AnalysisResult> {
  ...
  const modelName = "gemini-1.5-flash"; // or "gemini-1.5-flash"
...
${content}`;
    ...
    return analyzeWithAI(content); // Your fallback logic
  }
}
